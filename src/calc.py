...
def main() -> None:
    damage = pd.read_json('src/typs.json').transpose()
    pokes = pd.read_json('src/pokes.json').transpose()
    cnts = pd.read_json('src/typ_cnts.json').transpose()
    ...
    display = filtered.drop(['from_typs', 'to_typs'], axis=1).sort_values('rem', ascending=False)
    ...
    # -
    choice = best.loc['bug_fire']  # volcarona from larvesta at 59
    remaining = remaining - set(choice.to_typs)
    filtered['rem'] = filtered['to_typs'].map(lambda x: len(remaining.intersection(set(x))))
    display = filtered.drop(['from_typs', 'to_typs'], axis=1).sort_values('rem', ascending=False)
    ...
    # need grass, arboliva from dolliv -> smolliv

    # OR
    choice = best.loc['electric_poison']  # toxitricity
    remaining = remaining - set(choice.to_typs)
    filtered['rem'] = filtered['to_typs'].map(lambda x: len(remaining.intersection(set(x))))
    display = filtered.drop(['from_typs', 'to_typs'], axis=1).sort_values('rem', ascending=False)

    choice = best.loc['flying_water']  # gyrados
    remaining = remaining - set(choice.to_typs)
    filtered['rem'] = filtered['to_typs'].map(lambda x: len(remaining.intersection(set(x))))
    display = filtered.drop(['from_typs', 'to_typs'], axis=1).sort_values('rem', ascending=False)
